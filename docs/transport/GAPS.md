...
- **2026-09-12: Claude Code 2.1.269 also passed the packaged router checks.** The existing parser and driver were reused, with a version-specific generator record checked against the executed snapshot. Initial run `handler-nufNFS` passed M3-A and model selection. Native runs `next-turn-uOBFla` (including parallel), `nested-371OGp`, `compaction-fyvzkc`, and `resume-UHAJJ2` supplied the phase evidence. Packaged runs `next-turn-wqfgz6`, `nested-eHKU2Q`, `compaction-ffSI2i`, and `resume-bEB1Q3` passed their lifecycle, routing, completion, identity and pairing checks. No `src/` routing change was required. The [minimal example](../../examples/minimal-router/README.md) uses explicit `terra` and `sol` aliases with environment-only gateway settings.
- **Limits.** M2 is failed on 2.1.268 and remains unmeasured on 2.1.269. M3, M3-B2, M4, and M10-freshness remain unmeasured or closed. Correlation is in memory, expires after idle TTL, and is lost on restart. These limits do not invalidate the exercised explicit-marker and correlation paths; they limit what this profile claims to support.
