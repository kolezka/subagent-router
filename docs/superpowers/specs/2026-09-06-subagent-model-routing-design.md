...
- Testy D2 sprawdzają, że adapterowy marker w pozycji `user` jest `ignored-marker`, dopóki M3 nie potwierdzi tej dokładnej pozycji i tokenu. Każde użycie B wymaga zgodnego `agent_id` z nagłówkiem. Kanał B2 wymaga wyniku M1, `correlation: "auto"`, potwierdzonego pochodzenia każdego requestu i nie może ominąć guardu kanału C.
